...
DEFAULT_EXPECTED_HEADERS = {'Content-Type': 'application/json', 'Accept': 'application/json; charset=utf-8',
...
def test_get(mock_api_session_provider, mock_requests_session):
    client = HttpApiClient('https://api.example.com', 'https://api2.example.com', mock_api_session_provider)
    client.get('/datasets')
